...
namespace Broadcast {
    ...
    private:
            ...
            , _callback(nullptr)
        {
            ...
            if(errCode == NFE_RETURN_OK) {
                    ...
                    TRACE_L1("FE supported modes :%s%s%s%s%s%s%s%s%s%s\n"
                            , (supportedModes & NFE_MODE_DVBS                  ) ? "DVB-S " : ""
                            , (supportedModes & NFE_MODE_DVBS2                 ) ? "DVB-S2 " : ""
                            , (supportedModes & NFE_MODE_CABLE_INBAND_ANNEXE_A ) ? "DVB-C_Annex_A " : ""
                            , (supportedModes & NFE_MODE_CABLE_INBAND_ANNEXE_B ) ? "DVB-C_Annex_B " : ""
                            , (supportedModes & NFE_MODE_CABLE_OOB_A           ) ? "DVB-C_OOB_A " : ""
                            , (supportedModes & NFE_MODE_CABLE_OOB_B           ) ? "DVB-C_OOB_B " : ""
                            , (supportedModes & NFE_MODE_DVBT                  ) ? "DVB-T " : ""
                            , (supportedModes & NFE_MODE_DVBT2                 ) ? "DVB-T2 " : ""
                            , (supportedModes & NFE_MODE_ATSC                  ) ? "ATSC " : ""
                            , (supportedModes & NFE_MODE_ISDBT                 ) ? "DVB-ISDBT " : "");
            }
 
            if ((errCode != NFE_RETURN_OK) || (_stcChannel = NEXUS_SimpleStcChannel_Create(nullptr)) == nullptr) {
                TRACE_L1("Unable to find QAM-capable frontend: %d", __LINE__);
                fic = fopen("/var/tmp/flx.txt","a");
                fprintf(fic, "FLX Unable to find QAM-capable frontend\n ");
                fclose(fic);
    ...
    public:
        ...
        static ITuner* Create(const string& info)
        {
            ...
            TRACE_L1("FLX Tuner::Create %d", index);
            FILE *fic = fopen("/var/tmp/flx.txt","a");
    ...
    public:
        ...
        {
            uint32_t result = Core::ERROR_UNAVAILABLE;
            NFE_AsyncID       satAsyncID;


            FILE *fic = fopen("/var/tmp/flx.txt","a");
            fprintf(fic, "FLX Tune freq = %d\n ", frequency);
            fclose(fic);
                ...
                parserBandSettings.sourceTypeSettings.inputBand = 0; //FLX userParams.param1;
                ...
                if (NEXUS_ParserBand_SetSettings(_parserBand, &parserBandSettings) == 0) {
                    ...
                    switch (modulation) {
                    case QAM16:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_16;
                        break;
                    case QAM32:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_32;
                        break;
                    case QAM64:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_64;
                        break;
                    case QAM128:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_128;
                        break;
                    case QAM256:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_256;
                        break;
                    case QAM1024:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_1024;
                        break;
                    case QAM2048:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_2048;
                        break;
                    case QAM4096:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_4096;
                        break;
                    default:
                        _tuneParam.tune.cabIb.symbolRate  = NFE_QAM_256;
                        break;
                    ...
                    if(NFE_tune(_feHandle, &_tuneParam, &satAsyncID) != NFE_RETURN_OK)  {
                        TRACE_L1("Error: NFE_tune failure\n");
    ...
    private:
		...
		std::thread t(ThreadCallback, user);
                t.join();
            }
       }
...
*/
    ...
    {
        TRACE_L1("FLX call of Tuner::Create ");
        FILE *fic = fopen("/var/tmp/flx.txt","a");
            fprintf(fic, "FLX call of Tuner::Create\n");
            fclose(fic);
