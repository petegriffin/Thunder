...
ans(PLUGIN_WEBKITBROWSER_INFORMATION_TRACING)

map()
    kv(module "Plugin_WebKitBrowser")
    kv(category "Activity")
    kv(enabled true)
end()
ans(PLUGIN_WEBKITBROWSER_ACTIVITY_TRACING)

map()
    kv(module "Plugin_WebKitBrowser")
    kv(category "HTML5Notification")
    kv(enabled true)
end()
ans(PLUGIN_WEBKITBROWSER_HTML5Notification_TRACING)

map()
    kv(module "Plugin_Bluetooth")
    kv(category "Information")
    kv(enabled true)
end()
ans(PLUGIN_BLUETOOTH_TRACING_Information)
map()
    kv(module "Plugin_Bluetooth")
    kv(category "Error")
    kv(enabled true)
end()
ans(PLUGIN_BLUETOOTH_TRACING_Error)

map()
    kv(module "Plugin_RemoteControl")
    kv(category "Information")
    kv(enabled true)
end()
ans(PLUGIN_REMOTECONTROL_TRACING_Information)
map()
    kv(module "Plugin_RemoteControl")
    kv(category "KeyActivity")
    kv(enabled true)
end()
ans(PLUGIN_REMOTECONTROL_TRACING_KeyActivity)
map()
    kv(module "Plugin_RemoteControl")
    kv(category "UnknownKey")
    kv(enabled true)
end()
...
if (PLUGIN_BLUETOOTH_TRACING)
    map_append(${CONFIG} tracing ${PLUGIN_BLUETOOTH_TRACING_Information})
    map_append(${CONFIG} tracing ${PLUGIN_BLUETOOTH_TRACING_Error})
endif(PLUGIN_BLUETOOTH_TRACING)

if (PLUGIN_REMOTECONTROL_TRACING)
    map_append(${CONFIG} tracing ${PLUGIN_REMOTECONTROL_TRACING_Information})
